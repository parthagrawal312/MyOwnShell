...
vector<string> split_sentence(string input) {
      ...
      for (char c : input) {
        ...
        if(c=='\'' && opendoublequote==false && keepNextCharSafe==false)// Iterate through each character in the user input sentence
        {
          openquote=!openquote;
          continue;
        }
        if(c=='\"' && openquote==false && keepNextCharSafe==false)
        {
